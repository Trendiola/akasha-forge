            ...
            <div key={item.id} className="group flex flex-col rounded-xl border border-border bg-card/60 p-4 transition-colors hover:border-primary/40" data-testid={`forge-item-${item.id}`}>
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex shrink-0 gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                  <button onClick={() => openEdit(item)} className="text-muted-foreground hover:text-primary" data-testid={`forge-edit-${item.id}`}><Pencil className="h-3.5 w-3.5" /></button>
                  <button onClick={() => { del.mutate(item.id); toast.success(`${schema.singular} deleted`); }} className="text-muted-foreground hover:text-destructive" data-testid={`forge-delete-${item.id}`}><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
              </div>
